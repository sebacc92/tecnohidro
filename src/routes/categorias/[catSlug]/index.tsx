  ...
  if (!data.value) {
    return (
      <div class="container mx-auto px-4 py-20 text-center">
        ...
        <Link href="/productos" class="text-orange-600 underline">Ver catálogo completo</Link>
      </div>
    );
  }
        ...
        <Breadcrumb.List>
          ...
          {parentCategory && (
            <>
              <Breadcrumb.Separator />
              <Breadcrumb.Item>
                <Breadcrumb.Link href={`/categorias/${parentCategory.slug}`}>{parentCategory.name}</Breadcrumb.Link>
              </Breadcrumb.Item>
            </>
          )}
          <Breadcrumb.Separator />
          <Breadcrumb.Item><Breadcrumb.Page class="text-slate-800 font-medium">{category.name}</Breadcrumb.Page></Breadcrumb.Item>
        </Breadcrumb.List>
      </Breadcrumb.Root>

      <div class="mb-8 border-b pb-6">
        <h1 class="text-3xl md:text-4xl font-bold text-slate-900">{category.name}</h1>
        ...
        <aside class="w-full md:w-64 shrink-0">
          <div class="sticky top-24 bg-white p-5 rounded-xl border border-slate-200">
            <div class="flex items-center gap-2 mb-4 text-slate-800 font-semibold border-b pb-3">
              <LuFilter class="h-5 w-5" /><h2>Categorías</h2>
            </div>
            <ul class="space-y-2">
              ...
              {rootCats.map((cat) => {
                ...
                const isExpanded = cat.slug === category.slug || cat.slug === parentCategory?.slug;
                if (subs.length > 0) return (
                  <li key={cat.id} class="mb-1">
                    <details class="group" open={isExpanded}>
                      <summary class={`flex justify-between items-center cursor-pointer py-1.5 px-3 rounded-md text-sm list-none select-none ${isActive ? 'text-orange-700 font-semibold' : 'text-slate-700 font-medium hover:bg-slate-50'}`}>
                      ...
                      <ul class="pl-4 mt-1 border-l-2 border-slate-100 ml-4 space-y-1 mb-2">
                        {subs.map((sub) => (
                          <li key={sub.id}>
                            <Link href={`/categorias/${cat.slug}/${sub.slug}`} class={`block py-1 px-2 rounded text-xs transition-colors ${sub.slug === category.slug ? 'bg-orange-50 text-orange-700 font-medium' : 'text-slate-500 hover:bg-slate-50'}`}>
                              {sub.name}
                            </Link>
                          </li>
                        ))}
                ...
                return (
                  <li key={cat.id} class="mb-1">
                    <Link href={`/categorias/${cat.slug}`} class={`block py-1.5 px-3 rounded-md text-sm ${isActive ? 'bg-orange-50 text-orange-700 font-medium' : 'text-slate-700 font-medium hover:bg-slate-50'}`}>
                      {cat.name}
                    </Link>
                  </li>
                );
                ...
                if (viewMode.value === 'list') return (
                  <div key={product.id} class="bg-white rounded-xl border border-slate-200 hover:shadow-md transition-all flex flex-row overflow-hidden">
                    ...
                    <div style="width:180px" class="flex flex-col items-end justify-center gap-2 px-4 border-l border-slate-100 shrink-0">
                      ...
                      {inStock ? <span class="text-[11px] text-emerald-600 font-medium flex items-center gap-1"><LuCheck class="w-3 h-3" />En Stock</span> : <span class="text-[11px] text-amber-600 font-medium">Consultar stock</span>}
...
export const head: DocumentHead = ({ resolveValue }) => {
  const d = resolveValue(useCategoryData);
  if (!d) return { title: 'Tecnohidro' };
  return {
    title: `${d.category.name} - Catálogo | Tecnohidro`,
    meta: [{ name: 'description', content: `Productos de la categoría ${d.category.name} en Tecnohidro.` }],
  };
};
