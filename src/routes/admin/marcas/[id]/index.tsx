...
import { type DocumentHead, routeLoader$, routeAction$, Form, Link } from '@builder.io/qwik-city';
...
export const useBrand = routeLoader$(async ({ params, env, error }) => {
  const db = getDb(env);
  const data = await db.select().from(brands).where(eq(brands.id, params.id));
  if (data.length === 0) throw error(404, 'Marca no encontrada');
  return data[0];
});
...
export default component$(() => {
  ...
  const handleImageChange = $((event: Event) => {
    ...
    if (input.files && input.files[0]) {
      ...
      imagePreview.value = brand.value.imageUrl;
    }
  });
  ...
  return (
    <div class="max-w-3xl mx-auto">
      <div class="mb-6 flex items-center gap-4">
        ...
        <div>
          <h1 class="text-2xl font-bold text-slate-900">Editar Marca</h1>
          <p class="text-slate-500">Modifica los datos de la marca seleccionada.</p>
        </div>
      </div>
        ...
        <Form action={editAction} enctype="multipart/form-data" class="p-6 space-y-6">
          ...
          <div class="space-y-4">
            <div class="space-y-1.5">
              ...
              <input type="text" id="name" name="name" required value={brand.value.name} class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-cyan-500 focus:ring-cyan-500 outline-none" placeholder="Ej: Tigre, IPS, Bosch" />
            </div>
            ...
            <div class="space-y-1.5">
              ...
              <select id="category" name="category" required class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-cyan-500 focus:ring-cyan-500 outline-none bg-white">
                <option value="infraestructura" selected={brand.value.category === 'infraestructura'}>Línea Infraestructura</option>
                <option value="domiciliaria" selected={brand.value.category === 'domiciliaria'}>Línea Domiciliaria</option>
                <option value="herramientas" selected={brand.value.category === 'herramientas'}>Línea Herramientas</option>
              </select>
            </div>
