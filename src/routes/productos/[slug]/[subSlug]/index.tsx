...
import { LuExternalLink, LuTag, LuLayoutGrid, LuList, LuFilter, LuChevronDown } from '@qwikest/icons/lucide';
import { ProductImageCarousel } from '../../../../components/ProductImageCarousel';
import { ShareButton } from '../../../../components/ui/share-button';

export const useSubCatData = routeLoader$(async (requestEvent) => {
  ...
  try {
    ...
    if (!parentCat || !subCat) { requestEvent.status(404); return { found: false as const }; }
    ...
    return {
      ...
      allCategories, products: prods,
    };
  } catch (err) {
    console.error(err); requestEvent.status(500); return { found: false as const };
  }
});

export default component$(() => {
  const data = useSubCatData();
  const viewMode = useSignal<'grid' | 'list'>('grid');
  if (!data.value.found) {
    return <div class="container mx-auto px-4 py-20 text-center"><h1 class="text-2xl font-bold mb-4">Categoría no encontrada</h1><Link href="/productos" class="text-orange-600 underline">Ver catálogo</Link></div>;
  }
    ...
    <div class="container mx-auto px-4 md:px-8 py-12">
      <Breadcrumb.Root class="mb-6">
        <Breadcrumb.List>
          ...
          <Breadcrumb.Separator />
          <Breadcrumb.Item><Breadcrumb.Link href={`/productos/${parentCategory.slug}`}>{parentCategory.name}</Breadcrumb.Link></Breadcrumb.Item>
          <Breadcrumb.Separator />
          ...
          <div class="sticky top-24 bg-white p-5 rounded-xl border border-slate-200">
            ...
            <ul class="space-y-2">
              ...
              {rootCats.map((cat) => {
                ...
                const expanded = cat.slug === parentCategory.slug;
                  ...
                  <li key={cat.id} class="mb-1">
                    <details class="group" open={expanded}>
                      <summary class={`flex justify-between items-center cursor-pointer py-1.5 px-3 rounded-md text-sm list-none select-none ${cat.slug === parentCategory.slug ? 'text-orange-700 font-semibold' : 'text-slate-700 font-medium hover:bg-slate-50'}`}>
                        <span>{cat.name}</span><LuChevronDown class="h-4 w-4 transition-transform group-open:rotate-180 text-slate-400" />
                      </summary>
                      <ul class="pl-4 mt-1 border-l-2 border-slate-100 ml-4 space-y-1 mb-2">
                        <li><Link href={`/productos/${cat.slug}`} class="block py-1 px-2 rounded text-xs text-slate-500 hover:bg-slate-50">Todo {cat.name}</Link></li>
                        {subs.map((sub) => (
                          <li key={sub.id}>
                            <Link href={`/productos/${cat.slug}/${sub.slug}`} class={`block py-1 px-2 rounded text-xs transition-colors ${sub.slug === subCategory.slug ? 'bg-orange-50 text-orange-700 font-medium' : 'text-slate-500 hover:bg-slate-50'}`}>{sub.name}</Link>
                          </li>
                        ))}
                      </ul>
                    </details>
                ...
                return <li key={cat.id} class="mb-1"><Link href={`/productos/${cat.slug}`} class="block py-1.5 px-3 rounded-md text-sm text-slate-700 font-medium hover:bg-slate-50">{cat.name}</Link></li>;
              })}
            </ul>
          </div>
        </aside>
              ...
              {prods.map((product) => {
                ...
                if (viewMode.value === 'list') return (
                  <div key={product.id} class="bg-white rounded-xl border border-slate-200 hover:shadow-md transition-all flex flex-row overflow-hidden">
                    <Link href={`/productos/${product.slug}`} class="w-28 bg-slate-50 flex items-center justify-center overflow-hidden">
                    ...
                    <div style="width:180px" class="flex flex-col items-end justify-center gap-2 px-4 border-l border-slate-100">
                      {hasPrice && <span class="text-lg font-bold text-orange-600">${product.price!.toLocaleString('es-AR')}</span>}
...
export const head: DocumentHead = ({ resolveValue }) => {
  const d = resolveValue(useSubCatData);
  if (!d.found) return { title: 'Tecnohidro' };
  return {
